...
>>>>>>> 0f1004269802a9bf7773a6cf3d0abc7e53d0e786
	let title = '';
	let description = '';
	let frontImage = '';
	let frontContent = '';
...
<<<<<<< HEAD
	...
	function saveFlashcard() {
		if ((isInitialCard && (!title || !description || !frontContent || !backContent)) || (!isInitialCard && (!frontContent || !backContent))) {
	  ...
	  {#if $flashcards.length > 0}
		<div class="p-4 mb-4 border border-gray-400 rounded shadow">
		  {#each $flashcards.slice(currentIndex, currentIndex + 1) as card, index}
			...
			<button class="bg-gray-900 text-white px-4 py-2 rounded hover:bg-blue-600 mr-2" on:click={() => editFlashcard(currentIndex)}>Edit</button>
			<button class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-700" on:click={() => deleteFlashcard(currentIndex)}>Delete</button>
		  {/each}
		</div>
		<div class="flex justify-between">
		  <button class="bg-gray-900 text-white px-4 py-2 rounded hover:bg-blue-600" on:click={previousFlashcard} disabled={currentIndex === 0}>Previous</button>
		  <button class="bg-gray-900 text-white px-4 py-2 rounded hover:bg-blue-600" on:click={nextFlashcard} disabled={currentIndex === $flashcards.length - 1}>Next</button>
		</div>
	  {:else}
		<p>No flashcards available. Please add a new flashcard.</p>
	  {/if}
	  <button class="bg-gray-900 text-white px-4 py-2 rounded hover:bg-blue-600 mt-4" on:click={addNewFlashcard}>Add New Flashcard</button>
	</div>
  {/if}
